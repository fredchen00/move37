...
def best_action_value(grid, V, s):
  ...
  grid.set_state(s)
  action_value=np.empty([len(ALL_POSSIBLE_ACTIONS),1])
  # loop through all possible actions to find the best current action
  for indx,a in enumerate(ALL_POSSIBLE_ACTIONS):
    ...
    action_value[indx]=expected_r+GAMMA*expected_v
    
    
    
  best_a=ALL_POSSIBLE_ACTIONS[np.argmax(action_value)]
